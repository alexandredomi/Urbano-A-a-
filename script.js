...
sizes.forEach((size,i)=>{const button=document.createElement('button');button.type='button';button.className='size-card';button.innerHTML=`<strong>${size.name}</strong><span>${size.detail}</span><b>${money(size.price)}</b>`;button.onclick=()=>{selectedSize=i;[...sizeRoot.children].forEach(x=>x.classList.remove('active'));button.classList.add('active')};sizeRoot.append(button)});
function updateSelectionInfo(){document.querySelector('#includedCount').textContent=`${selectedIncluded.length}/3`;document.querySelector('#extrasCount').textContent=`${selectedExtras.length}/3`}
function addOption(root,item,type){const name=typeof item==='string'?item:item.name,price=typeof item==='string'?null:item.price;const button=document.createElement('button');button.type='button';button.className='option';button.innerHTML=`<span>${name}</span>${price?`<small>+ ${money(price)}</small>`:''}<i class="check">✓</i>`;button.onclick=()=>{const array=type==='included'?selectedIncluded:selectedExtras,pos=array.indexOf(name);if(pos>=0){array.splice(pos,1);button.classList.remove('active')}else if(array.length>=3){showToast(type==='included'?'Você pode escolher até 3 complementos grátis.':'Você pode escolher até 3 extras pagos.');return}else{array.push(name);button.classList.add('active')}updateSelectionInfo()};root.append(button)}
included.forEach(item=>addOption(includedRoot,item,'included'));extras.forEach(item=>addOption(extrasRoot,item,'extra'));
function currentOrder(){if(selectedSize===null)return null;const size=sizes[selectedSize],selectedExtraData=extras.filter(item=>selectedExtras.includes(item.name));return{id:crypto.randomUUID?crypto.randomUUID():Date.now().toString(),name:`Açaí ${size.name}`,detail:size.detail,included:[...selectedIncluded],extras:selectedExtraData,note:document.querySelector('#notes').value.trim(),qty:1,price:size.price+selectedExtraData.reduce((sum,item)=>sum+item.price,0)}}
function persistCart(){try{localStorage.setItem(CART_STORAGE_KEY,JSON.stringify(cart))}catch{}}
function renderCart(){persistCart();const root=document.querySelector('#cartItems'),count=cart.reduce((sum,item)=>sum+item.qty,0),total=cart.reduce((sum,item)=>sum+item.price*item.qty,0);document.querySelector('#cartCount').textContent=count;document.querySelector('#total').textContent=money(total);document.querySelector('#sendOrder').disabled=!cart.length;if(!cart.length){root.innerHTML='<p class="empty">Sua sacola está vazia.<br>Monte um açaí e adicione aqui.</p>';return}root.innerHTML=cart.map(item=>`<article class="cart-item"><div class="cart-item-head"><div><strong>${item.name}</strong><small>${item.detail}</small></div><button class="remove" data-id="${item.id}" aria-label="Remover">×</button></div><p>${item.included.length?item.included.join(', '):'Sem complementos'}${item.extras.length?`<br><b>Extras:</b> ${item.extras.map(extra=>extra.name).join(', ')}`:''}${item.note?`<br><b>Obs.:</b> ${item.note}`:''}</p><div class="cart-bottom"><div class="quantity"><button data-action="minus" data-id="${item.id}" aria-label="Diminuir">−</button><span>${item.qty}</span><button data-action="plus" data-id="${item.id}" aria-label="Aumentar">+</button></div><strong>${money(item.price*item.qty)}</strong></div></article>`).join('')}
function resetBuilder(){selectedSize=null;selectedIncluded=[];selectedExtras=[];[...sizeRoot.children,...includedRoot.children,...extrasRoot.children].forEach(button=>button.classList.remove('active'));document.querySelector('#notes').value='';updateSelectionInfo()}
document.querySelector('#addToCart').onclick=()=>{const order=currentOrder();if(!order){showToast('Escolha o tamanho do seu açaí primeiro.');return}cart.push(order);renderCart();resetBuilder();toggleCart(false);showToast('Açaí adicionado à sua sacola!')};
